...
// Import the worker client that wraps your Comlink-exposed API
...
export function SecondPanel() {
  ...
  // Instantiate the worker client on component mount.
  ...
  // Example petitions array with plain values (not Field objects)
  ...
  const handleSignPetition = async (petitionIndex: number) => {
    ...
    try {
      // Prepare a JSON-serializable petition object.
      const dataToSend = {
        action: "sign",
        petition: {
      ...
      console.log("[UI] callWorker data:", JSON.stringify(dataToSend));
      // Since our worker expects a JSON string, we stringify here.
      const result = await zkappWorkerClient.remoteApi.signPetition(JSON.stringify(dataToSend), rawAddress);
      console.log("[UI] Worker responded:", result);
      ...
      console.error("[UI] Transaction failed:", error);
